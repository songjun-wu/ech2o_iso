...
# How many threads used? added by Songjun
parser.add_option("--nthreads",dest="nthreads",metavar="nthreads",help="how many threads used in this job?")
...
parser.add_option("--FWD_It",dest="FWD_It",metavar="FWD_It",help="Ensemble iteration number")

# If mode == 'sensi_morris'
# Only generating trajectories ?
parser.add_option("--MSinit",dest="MSinit",metavar="MSinit",
                  help="Switch for Morris sensitivity: 0=runs, 1=trajectories generation")
parser.add_option("--MSspace",dest="MSspace",metavar="MSspace",
                  help="Walk in the parameter space for Morris: 'trajectory' or 'radial'")
parser.add_option("--NumTraj",dest="nr",metavar="nr",
                  help="Set the number of trajectories for Morris")
...
if Config.runECH2O == 1:
    ...
    Config.cmde_ech2o = ' '.join([Config.tcmd,'OMP_NUM_THREADS='+str(options.nthreads),
                                  './'+exe_ech2o,cfg_ech2o])

    # Spinup? (depreciated)
    if Config.mode == 'forward_runs' and Config.spinup > 0:
        Config.spin_ech2o = ' '.join([Config.tcmd,'OMP_NUM_THREADS='+str(options.nthreads),
...
if Config.runECH2O == 1:
    ...
    with open(os.path.join(Config.PATH_OUT, cfg_ech2o),'a') as fw:
        ...
        fw.write('ClimateZones = ClimZones.map\n')
        fw.write('Isohyet_map = isohyet.map\n')
        if Config.isTrck == 1:
            fw.write('Tracking = 1\n')
...
for par in Paras.names:
    ...
    Opti.ind = Opti.ind+list(np.repeat(ipar,nr))  #index of parameter list [0,0,0,0,1,1,1,1,2,3,3,3,3,...]
    ipar+=1
...
if Config.mode == 'calib_runs':
    ...
    for it in range(it0,Opti.nit):
        ...
        # Create the inputs for ECH2O, and check if porosity L3 < residual soil moisture
